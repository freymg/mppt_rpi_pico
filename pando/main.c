...
float mppt_pando(float voltage, float power)
{
	float deltaPower = 0.0;
	...
	deltaPower = power - previousPower;
...
int main()
{
    ...
    while(true)
    {
        ...
        i2c_write_blocking(I2C_PORT, addr, &power_dir, 1, false);
        i2c_read_blocking(I2C_PORT, addr, power_i2c, 2, false);
        power_val = ((power_i2c[0] << 8) | power_i2c[1])*0.24;
        ...
        level = (unsigned int) mppt_pando(voltage_val, power_val);
